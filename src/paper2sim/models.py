...
class AtomicTopic(BaseModel):
    ...
    name: str = Field(description="A clear, descriptive heading for this topic (e.g., 'Self-Attention Mechanism', 'Positional Encoding')")
    summary: str = Field(description="A 2-3 sentence summary explaining the core idea, suitable as a video hook/introduction")
    full_explanation: str = Field(
        description=(
            "Complete, self-contained markdown text covering this topic. Must include all"
            " necessary context, definitions, equations, and explanations so a viewer can"
            " understand this topic WITHOUT having seen other topics. Reference figures and"
            " tables by their numbers with descriptions."
        )
    )
    key_takeaways: list[str] = Field(description="2-4 bullet points of the most important things to remember")
...
class Breakdown(BaseModel):
    ...
    document_summary: str = Field(description="Brief 2-3 sentence overview of the entire document")
    topics: list[AtomicTopic] = Field(description="List of atomic, self-contained topics extracted from the document")
...
class Scene(BaseModel):
    ...
    title: str = Field(description="A brief title for this scene (e.g., 'The Attention Spotlight', 'Matrix Multiplication Dance')")
    ...
    def to_text(self) -> str:
        ...
        return f"## [{self.scene_type}] {self.title}\n\n**Visual Description:**\n{self.visual_description}\n**Narration:**\n{self.narration}\n"
...
class TopicStoryboard(BaseModel):
    ...
    visual_concept: str = Field(description="The overarching visual metaphor or analogy that ties the entire storyboard together. What is the 'big picture' visual idea?")
    scenes: list[Scene] = Field(description="Ordered scenes (hook → mid scenes → closing) that tell the complete visual story. Each scene must have exhaustively detailed visual descriptions.")
